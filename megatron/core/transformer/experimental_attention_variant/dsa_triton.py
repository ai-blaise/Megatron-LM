...
class SparseDSAAttentionTriton(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.save_for_backward(query_flat, key_flat, value_flat, topk_flat, output, lse)
    ...
    def backward(ctx, grad_output: torch.Tensor):
        query, key, value, topk_indices, output, lse = ctx.saved_tensors
        q_len = query.shape[0]
        sk = key.shape[0]
...
def sparse_dsa_attention_triton(
    ...
    return SparseDSAAttentionTriton.apply(query, key, value, topk_indices, softmax_scale, q_start)
