...
def validate_model_config_args_from_heterogeneous_config(args):
    ...
    assert all(num == n_kv_heads_in_group[0] for num in n_kv_heads_in_group), (
    ...
    args_to_validate = {
        ...
        "num_query_groups": hf_config_dict.num_attention_heads
        // n_kv_heads_in_group[0],
    }
...
def validate_args(args, defaults={}):
    ...
    if args.non_persistent_ckpt_type == "local":
        try:
            ...
            raise RuntimeError(
                "nvidia_resiliency_ext is required for local checkpointing"
            ) from e
    ...
    if args.perform_rl_step:
        ...
        if (
            ...
            and args.rl_kv_cache_management_mode != "persist"
        ):
            try:
        ...
        assert not (
            not args.rl_persist_cuda_graphs
            and args.rl_kv_cache_management_mode == "offload"
        ...
        if (
            ...
            and args.rl_inference_expert_model_parallel_size
        ...
        args.grpo_samples_per_iteration = (
        ...
        if args.exit_interval is not None:
            assert (
                args.exit_interval % num_training_iterations_per_inference_iteration
                == 0
            ), (
        ...
        if args.save_interval is not None:
            assert (
                args.save_interval % num_training_iterations_per_inference_iteration
                == 0
            ), (
    ...
    if args.hierarchical_context_parallel_sizes:
        ...
        assert args.context_parallel_size == prod(
            args.hierarchical_context_parallel_sizes
        )
    ...
    if args.enable_cuda_graph or args.external_cuda_graph:
        ...
        assert not args.enable_cuda_graph or not args.external_cuda_graph, (
            "--enable-cuda-graph and --external-cuda-graph cannot be enabled at the same time."
        )

        if args.enable_cuda_graph:
            print_rank_0(
                "--enable-cuda-graph is deprecated, use --cuda-graph-impl=local instead.",
                args.rank,
            )
    ...
    for key in defaults:
        ...
        if getattr(args, key, None) is not None:
            warn_rank_0(
                "Overriding default arguments for {key}:{v} with {key}:{v2}".format(
    ...
    if (
        ...
        and (
            args.hybrid_override_pattern is None
            or sep not in args.hybrid_override_pattern
        )
    ):
        ...
        print_rank_0(
            f"Converted legacy MTP pattern to unified: {args.hybrid_override_pattern}"
        )
    ...
    if args.mtp_num_layers:
        ...
        assert (
            ...
            or args.position_embedding_type == "none"
        ), (
    ...
    if args.is_hybrid_model:
        ...
        if args.mtp_num_layers and not (
            args.hybrid_override_pattern and sep in args.hybrid_override_pattern
        ):
    ...
    if args.fp4:
        ...
        if te_ver is None:
            ...
            warnings.warn(
                "--fp4-format is set but Transformer Engine is not installed. "
                "FP4 features will use fallback implementations. "
                "Install Transformer Engine >= 2.7.0.dev0 for optimal FP4 support."
            )
        elif not is_te_min_version("2.7.0.dev0"):
            raise ValueError(
    ...
    if (
        ...
        raise ValueError(
            "MXFP8 with inference optimized layers requires FlashInfer >= 0.6.4"
        )
    ...
    if args.use_megatron_fsdp:
        ...
        if args.data_parallel_sharding_strategy in [
            "optim_grads_params",
            "optim_grads",
        ]:
            warn_rank_0(
    ...
    if args.fsdp_manual_registration:
        ...
        assert args.nccl_ub, (
            "FSDP manual registration is only supported with nccl-ub option"
        )
    ...
    if args.no_weight_decay_cond_type is not None:
        ...
        if args.no_weight_decay_cond_type == "apply_wd_to_qk_layernorm":
            ...
            raise ValueError(
                f"Invalid no_weight_decay_cond_type: {args.no_weight_decay_cond_type}"
            )
    ...
    if args.hybrid_context_parallel:
        assert not args.pipeline_model_parallel_size > 1, (
            "Hybrid context parallelism not supported with pipeline parallelism"
        )
        ...
        assert not args.use_megatron_fsdp, (
            "Hybrid context parallelism not supported with Megatron FSDP"
        )
        assert args.dataloader_type == "single", (
            "Hybrid context parallelism only supported with single dataloader type"
        )
        assert args.calculate_per_token_loss, (
            "Hybrid context parallelism must be used with --calculate-per-token-loss"
        )

    if args.sequence_packing_scheduler is not None:
        assert (
            args.context_parallel_size * args.max_seqlen_per_dp_cp_rank
            >= args.seq_length
        ), (
    ...
    if (
        ...
        if (
            ...
            and "ep_dp" not in args.high_priority_stream_groups
        ):
    ...
    if args.num_experts is not None and args.moe_ffn_hidden_size is None:
        ...
        warn_rank_0(
            "moe_ffn_hidden_size is not set, using ffn_hidden_size for MoE instead."
        )
    ...
    if args.fim_data:
        ...
        assert all(token is not None for token in extra_tokens), (
    ...
    if args.deterministic_mode:
        assert not args.use_flash_attn, (
            "Flash attention can not be used in deterministic mode."
        )
        ...
        assert (
            os.getenv("NCCL_ALGO", -1) != -1
            and os.getenv("NCCL_ALGO") in all_reduce_choices
    ...
    if args.result_rejected_tracker_filename is not None:
        ...
        iterations_to_skip_from_file = (
            RerunStateMachine.get_skipped_iterations_from_tracker_file(
    ...
    if args.fake_process_group:
        ...
        args.check_for_nan_in_loss_and_grad = False
        warn_rank_0(
            "check_for_nan_in_loss_and_grad is set to False for fake process group."
        )
        # Disable gloo process groups for fake process group
        args.enable_gloo_process_groups = False
        warn_rank_0(
            "enable_gloo_process_groups is set to False for fake process group."
        )
    ...
    if args.load_main_params_from_ckpt:
        assert args.no_load_optim, (
            "--load-main-params-from-ckpt must be used with --no-load-optim."
        )
    ...
    if args.moe_use_upcycling:
        assert args.save is not None, (
            "When using upcycling, the --save option must be specified."
        )
    ...
    if args.optimizer in ("muon", "dist_muon"):
        ...
        assert not args.overlap_grad_reduce, (
            "Muon optimizer does not support overlap grad reduce for now."
        )
        assert not args.overlap_param_gather, (
            "Muon optimizer does not support overlap param gather for now."
        )
        assert not args.use_torch_fsdp2, (
            "Muon optimizer does not support Torch-FSDP2 for now."
        )
        assert not args.use_megatron_fsdp, (
            "Muon optimizer does not support Megatron-FSDP for now."
        )
        ...
        assert args.experimental_attention_variant is None, (
            "Muon optimizer does not support attention variant for now."
        )
        assert not args.attention_output_gate, (
            "Muon optimizer does not support attention output gate for now."
        )
    ...
    if args.replication:
        ...
        warn_rank_0(
            "--replication-jump was specified despite not using replication. Ignoring."
        )
        ...
        if args.overlap_grad_reduce:
            ...
            if wgrad_in_graph_scope:
                ...
                if CudaGraphScope.attn in args.cuda_graph_scope:
                    assert not args.add_bias_linear and not args.add_qkv_bias, (
    ...
    if args.cuda_graph_impl != "none":
        if (
            ...
            warn_rank_0(
                "te_rng_tracker is not enabled, enabling it for CUDA graphs.", args.rank
            )
        if args.cuda_graph_impl == "transformer_engine":
            assert (
                "expandable_segments:True"
                not in os.getenv("PYTORCH_CUDA_ALLOC_CONF", "")
    ...
    if args.cuda_graph_scope == "full" or (
        ...
        if isinstance(args.cuda_graph_scope, list):
            assert args.cuda_graph_scope == ["full"], (
    ...
    if args.moe_latent_size is not None:
        ...
        assert not args.moe_use_legacy_grouped_gemm, (
            "MoE latent projection is not supported yet with legacy grouped GEMM."
        )
...
def core_transformer_config_from_args(args, config_class=None):
    ...
    if hasattr(args, "kitchen_config_file") and args.kitchen_config_file is not None:
        ...
        hasattr(args, "kitchen_recipe_number")
        and args.kitchen_recipe_number is not None
    ):
        kw_args["use_kitchen"] = True
        kw_args["quant_recipe"] = kitchen_quantization_recipe_config(
            args.kitchen_recipe_number
        )
...
def _add_inference_args(parser):
    ...
    group.add_argument(
        "--inference-flask-server-logging",
...
def _add_regularization_args(parser):
    ...
    group.add_argument(
        "--no-flash-adamw-fused",
        action="store_false",
        dest="flash_adamw_fused",
        help="FlashAdamW: disable fused Triton kernel. Useful for debugging.",
    )
    group.add_argument(
        "--flash-adamw-eco",
        action="store_true",
...
def _add_rl_args(parser):
    ...
    group.add_argument(
        "--perform-rl-step", action="store_true", help="Use the RL training step."
    )
    group.add_argument(
        "--rl-prompts-per-eval",
        type=int,
...
def _add_training_args(parser):
    ...
    prof_factory = ArgumentGroupFactory(
        ProfilingConfig, exclude=["record_shapes", "nvtx_ranges"]
    )
...
def _add_rerun_machine_args(parser):
    ...
    rerun_factory = ArgumentGroupFactory(
        RerunStateMachineConfig, exclude=["check_for_nan_in_loss"]
    )
...
def _add_learning_rate_args(parser):
    ...
    sched_factory = ArgumentGroupFactory(
        SchedulerConfig, exclude=["no_weight_decay_cond_type"]
    )
...
def _add_data_args(parser):
    ...
    group.add_argument(
        "--fim-data", action="store_true", help="Whether to use the FIM dataset."
    )
    group.add_argument(
        "--fim-rate",
        type=float,
    ...
    group.add_argument(
        "--fim-pad-token", type=str, default="<fim_pad>", help="FIM PAD token"
    )
    group.add_argument(
        "--fim-eod-token", type=str, default="<|endoftext|>", help="FIM EOD token"
    )
    return parser
