...
PIPELINE_MODEL_PARALLEL_LAYOUT="${PIPELINE_MODEL_PARALLEL_LAYOUT:-Et*5|t*5|t*4|t|t*5|t*5|t*5|t|t*5|t*5|t*4|t|t*5|t*5|t*4|tL}"
...
EP overlap:    overlap_moe=${OVERLAP_MOE_EXPERT_PARALLEL_COMM:-0} high_priority_a2a=${HIGH_PRIORITY_A2A_COMM_STREAM:-0} p2p_warmup_flush=${OVERLAP_P2P_COMM_WARMUP_FLUSH:-1} p2p_prewarm=${MEGATRON_PREWARM_PIPELINE_P2P:-1} early_attn_release=${EP_OVERLAP_EARLY_ATTN_MEMORY_RELEASE:-0} delay_wgrad=${DELAY_WGRAD_COMPUTE:-0}
...
StreamBP LM:   fused_lce=${MEGATRON_STREAMBP_FUSED_LCE:-1} logits_chunk=${STREAMBP_LOGITS_CHUNK_SIZE:-8192} lce_sp_tile=${MEGATRON_STREAMBP_FUSED_LCE_SP_TILE_SIZE:-1024} lce_fwd_split=${LCE_FWD_VOCAB_SPLIT_SIZE:-8192} lce_bwd_split=${LCE_BWD_VOCAB_SPLIT_SIZE:-8192}
StreamBP MoE:  chunk_forward=${STREAMBP_MOE_CHUNK_FORWARD:-0} mlp_chunks=${STREAMBP_MOE_MLP_CHUNKS:-4} bwd_mlp_chunks=${STREAMBP_MOE_MLP_BACKWARD_CHUNKS:-16} split_mlp_attn_bwd=${MEGATRON_STREAMBP_SPLIT_MOE_MLP_ATTENTION_BACKWARD:-1} skip_pp=${MEGATRON_STREAMBP_SKIP_PP_RANKS-}
...
MoE sort trim: enabled=${MEGATRON_MOE_SORT_TRIM_CACHE:-1} free_mb=${MEGATRON_MOE_SORT_TRIM_FREE_MB:-65536} safety_mb=${MEGATRON_MOE_SORT_TRIM_SAFETY_MB:-1024} cached_mb=${MEGATRON_MOE_SORT_TRIM_CACHED_MB:-512}
...
Activation offload: enabled=${FINE_GRAINED_ACTIVATION_OFFLOADING:-1} modules="${OFFLOAD_MODULES:-expert_fc1 core_attn attn_proj}"
Activation ECO: enabled=${NVFP4_ACTIVATION_ECO:-1} modules="${NVFP4_ACTIVATION_ECO_MODULES:-all}" recompute_only=${NVFP4_ACTIVATION_ECO_RECOMPUTE_ONLY:-1} backend=${NVFP4_ACTIVATION_ECO_QUANTIZER_BACKEND:-te} correction_dtype=${NVFP4_ACTIVATION_ECO_CORRECTION_DTYPE:-fp32}
...
export OVERLAP_P2P_COMM_WARMUP_FLUSH="${OVERLAP_P2P_COMM_WARMUP_FLUSH:-1}"
...
export STREAMBP_CHUNK_SIZE="${STREAMBP_CHUNK_SIZE:-4096}"
...
export STREAMBP_MOE_MLP_BACKWARD_CHUNKS="${STREAMBP_MOE_MLP_BACKWARD_CHUNKS:-16}"
...
export DSA_CHUNK_SIZE="${DSA_CHUNK_SIZE:-4096}"
...
export OFFLOAD_MODULES="${OFFLOAD_MODULES:-expert_fc1 core_attn attn_proj}"
...
export NVFP4_ACTIVATION_ECO_CORRECTION_DTYPE="${NVFP4_ACTIVATION_ECO_CORRECTION_DTYPE:-fp32}"
