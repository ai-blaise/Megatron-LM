#!/bin/bash
# DeepSeek-V3.2 MoE SFT with 2.5-bit TurboQuant on the dense MLA latent KV.
#
...
# name). This script REPLACES that scheme with TurboQuant 2.5-bit fake-quant
# applied at the post-LayerNorm 512-dim MLA latent. SpinQuant K/V bits are
# left at their defaults but the SpinQuant KV path is not enabled, so only
# TurboQuant runs on the dense KV. The DSA Indexer's IndexerK8 path is
# unaffected (out of scope per project specification).
#
# To run baseline NVFP4 weights without TurboQuant, use run_sft_deepseek_nvfp4.sh.

#SBATCH --job-name=deepseek_turboquant_sft
...
CHECKPOINT_PATH=${1:-"$HOME/checkpoints/sft_deepseek_turboquant"}
TENSORBOARD_LOGS_PATH=${2:-"$HOME/tensorboard_logs/sft_deepseek_turboquant"}
...
# ======================
# TurboQuant — 2.5-bit on the dense MLA-latent KV
# Replaces the model's KV4 scheme. Per-layer signs/codebooks are seeded
# deterministically from (turboquant-kv-seed, layer_idx); all TP/SP/CP/EP
# ranks construct identical buffers without collective communication.
# ======================
TURBOQUANT_ARGS=(
